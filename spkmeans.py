...
## goal == spk
    ...
    # leave the k first colomns
    vector_matrix = py_jacobi_matrix[:,0:k]
    ## add index colomn
    vector_matrix_with_ind = np.zeros((n,k+1))
    for i in range (n):
        for j in range(k + 1):
            if j==0:
                vector_matrix_with_ind[i][j] = i
            else:
                vector_matrix_with_ind[i][j] = vector_matrix[i][j-1]
    ...
    final_centroids = spkmeansmodule.fit(vector_matrix.tolist(), matrix[:,1:k+1].tolist(), 6)
